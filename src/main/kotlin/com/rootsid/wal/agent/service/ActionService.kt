...
class ActionService(
    ...
    private val log = LoggerFactory.getLogger(ActionService::class.java)
    ...
    fun receiveMessage(payload: ReceiveMessageRequest): ReceiveMessageResponse {
        ...
        receivedMsg.from?.let { from ->
            ...
            val myConn = didCommConnectionDataProvider.findByMyDid(receivedMsg.to)
            persistMessageReceived(
                connectionId = myConn._id,
                receivedPtMsg = receivedPtMsg,
                to = receivedMsg.to,
                from = from
            )
